...
def get_relevant_doses_with_bp(inputevents, chartevent):
    ...
    inputevents_states_ok = inputevents_states_ok[
        ["stay_id", "starttime", "endtime", "rate", "statusdescription", "originalrate", "itemid_label",
         "State"]].sort_values(by=["stay_id", "starttime"])
    ...
    tqdm.pandas()
    inputevents_states_ok[["bp_time", "bp_val"]] = inputevents_states_ok.progress_apply(
        lambda row: get_nearest_bp(row, chartevent), axis=1, result_type="expand")
